...
class SMSParsingService:
    ...
    def __init__(self):
        ...
        self.duration_patterns = [
            re.compile(r'(\d+)\s*(?:hours?|hrs?|h)\s*(?:and\s*)?(?:(\d+)\s*(?:minutes?|mins?|m))?', re.IGNORECASE),
    ...
    def parse_sms_to_activity(self, sms_message: SMSMessage) -> Optional[Activity]:
        ...
        try:
            ...
            activity_type = self._extract_activity_type(message_body, sms_message.keyword)
            ...
            description = self._create_description(message_body, activity_type, duration_minutes, location)
            ...
            return Activity(
                ...
                metadata={
                    ...
                    'parsing_confidence': self._calculate_confidence(message_body, activity_type)
                }
            )
    ...
    def _extract_activity_type(self, message_body: str, keyword: Optional[str] = None) -> ActivityType:
        """
        ...
        type_scores: Dict[ActivityType, int] = {activity_type: 0 for activity_type in ActivityType}
            ...
            if match:
                ...
                if len(groups) >= 2 and groups[1]:  # Hours and minutes
                    ...
                    if 'hour' in match.group().lower() or 'hr' in match.group().lower():
                        return value * 60
                    elif 'minute' in match.group().lower() or 'min' in match.group().lower():
                        return value
                    elif 'second' in match.group().lower() or 'sec' in match.group().lower():
    ...
    def _create_description(self, message_body: str, activity_type: ActivityType, 
                          duration_minutes: Optional[int], location: Optional[str]) -> str:
        """
        ...
        description = re.sub(r'^[,\-\s]+|[,\-\s]+$', '', description)  # Remove leading/trailing punctuation
    ...
    def _calculate_confidence(self, message_body: str, activity_type: ActivityType) -> float:
        """
        ...
        matching_keywords = [
            keyword for keyword, keyword_type in self.activity_keywords.items()
    ...
    def get_parsing_suggestions(self, message_body: str) -> List[str]:
        ...
        has_activity_keyword = any(
            ...
            for keyword in self.activity_keywords.keys()
        )
        ...
        # Check for duration
        has_duration = any(
            pattern.search(message_body) 
            for pattern in self.duration_patterns
        )
        
        if not has_duration:
            suggestions.append(
                "Include duration like 'for 30 minutes' or '2 hours'"
            )
        ...
        if not has_location:
            suggestions.append(
                "Include location like 'at office' or 'in gym'"
            )
        
        # Check message length
        if len(message_body.split()) < 3:
            suggestions.append(
                "Provide more details about the activity"
            )
