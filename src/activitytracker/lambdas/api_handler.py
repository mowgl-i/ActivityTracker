...
def _handle_get_activities(
    activity_service: ActivityService, query_params: Dict[str, str]
) -> Dict[str, Any]:
    """
...
def _handle_get_activity(
    activity_service: ActivityService, activity_id: Optional[str]
) -> Dict[str, Any]:
    """
    ...
    try:
        if not activity_id:
            return _create_error_response(
                400, "Missing Activity ID", "Activity ID is required"
            )
        ...
        if not activity:
            return _create_error_response(
                404, "Activity Not Found", f"Activity with ID '{activity_id}' not found"
            )
...
def _handle_create_activity(
    activity_service: ActivityService, body: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not body or body.strip() == "":
            return _create_error_response(
                400, "Missing Request Body", "Request body is required"
            )
        ...
        try:
            ...
            return _create_error_response(
                400, "Invalid JSON", f"Request body is not valid JSON: {str(e)}"
            )
        ...
        for field in required_fields:
            if field not in data or not data[field]:
                return _create_error_response(
                    400, "Missing Required Field", f"Field '{field}' is required"
                )
        ...
        if activity_service.db_service.save_activity(activity):
            ...
            return _create_error_response(
                500, "Failed to Save Activity", "Could not save activity to database"
            )

    except Exception as e:
        _log_api_error(
            "CREATE_ACTIVITY_ERROR", str(e), {"body": body[:200] if body else None}
        )
...
def _handle_get_stats(
    ...
    try:
        ...
        if days < 1 or days > 365:
            return _create_error_response(
                400, "Invalid Days Parameter", "Days must be between 1 and 365"
            )
        ...
        if "error" in stats:
            return _create_error_response(
                500, "Failed to generate statistics", stats["error"]
            )

        response_data = {
            "statistics": stats,
            "filters": {"phone": phone_number, "days": days},
            "timestamp": datetime.utcnow().isoformat(),
        }
...
def _handle_cors_preflight() -> Dict[str, Any]:
    ...
    """
    return {"statusCode": 200, "headers": _get_cors_headers(), "body": ""}
...
def _create_response(status_code: int, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "headers": {**_get_cors_headers(), "Content-Type": "application/json"},
        "body": json.dumps(data, indent=2, default=str),
    }


def _create_error_response(
    status_code: int, error: str, details: str = ""
) -> Dict[str, Any]:
    """
...
def _log_api_error(
    ...
    try:
        ...
        if context:
            ...
            safe_context = {
                k: v for k, v in context.items() if k not in ["phone_number", "body"]
            }
